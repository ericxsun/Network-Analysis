function T = decentralized_search_2D(L, adjL, src, dst, pbc)
%DECENTRALIZED_SEARCH_2D To compute the delivery time on 2-D lattice.
%   T = DECENTRALIZED_SEARCH_2D(L, adjL, src, dst, pbc) returns the delivery 
%   time of the message forward from src to dst on the 2D lattice defined by the
...
%   845-845.
%
%   See also: DECENTRALIZED_SEARCH_1D
%
...
if ~isempty(find(not(cellfun(@isempty, adjL)), 1))
    while 1
        ...
        %forward the message from src to its neighbor whichi is the nearest one 
        ...
        gty = pbc .* (yNsrc > ydst + dist_max) + not(pbc) .* yNsrc;
        lty = pbc .* (yNsrc < ydst - dist_max) + not(pbc) .* yNsrc;
        
        xNsrc(gtx>0) = xNsrc(gtx>0) - L;
        xNsrc(ltx>0) = xNsrc(ltx>0) + L;
        yNsrc(gty>0) = yNsrc(gty>0) - L;
        yNsrc(lty>0) = yNsrc(lty>0) + L;
        
        dist = abs(xdst - xNsrc) + abs(ydst - yNsrc);
