function edgeL = rewire_assort(edgeL, k, components_func, p)
%REWIRE_ASSORT Make the degree of connected nodes more similar by rewiring 
%the edge while preserving he degree sequence and connectivity.
%   edgeL = REWIRE_ASSORT(edgeL, k, components_func, p) returns the much
%   assortative graph.
%
%   Algorithm:
%   Iteratively rewire the randomly chosen two edges with probability p, 
%   subjects to that: nodes with higher degree should be linked, in k steps.
%
%   Note: 
...
%   Ref:
%
%   See also: REWIRE_DISASSORT, REWIRE_RAND, REWIRE_SA
%
...
while rew < k
    ...
    id2 = deg_s(2, 1);
    id1 = deg_s(1, 1);
    exist43 = sum(edgeL(:, 1) == id4 & edgeL(:, 2) == id3);
    exist34 = sum(edgeL(:, 1) == id3 & edgeL(:, 2) == id4);
    exist21 = sum(edgeL(:, 1) == id2 & edgeL(:, 2) == id1);
    exist12 = sum(edgeL(:, 1) == id1 & edgeL(:, 2) == id2);
